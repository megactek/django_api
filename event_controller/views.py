...
class EventMainView(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not features:
            AddressGlobal.objects.filter(id=a_serializer.data['id']).delete()
            EventMain.objects.filter(id=e_serializer.data['id']).delete()
...
class EventAttenderView(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        evt = EventMainView.queryset.filter(
            id = at_serializer.validated_data.get('eventmain_id', None)
        )
        if not evt:
        ...
        #check if the maximum sear has not been reached
        at_count = self.queryset.filter(eventmain_id=evt.id).count()
        if not at_count < evt.max_seat:
            raise Exception('Maximum attenders exceeded')
        ...
        at_serializer.save()
        return Response(at_serializer.data, status=200)
